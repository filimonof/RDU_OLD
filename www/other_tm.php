...
define("cod_company","8");

session_start();

function assemble_list_OtherCompany(&$lst,$write=false){

	$lst='<table  bgcolor="'.def_color_desktop_bgcolor.'" width="600" border="0" cellpadding="3" cellspacing="0"  bordercolor="'.def_color_bg.'" align="center">';
	
	if(QueryScript("select * from tblOtherCompany order by orders,name",$result,$connect)){

		$number=0;
		while ($comp=mysql_fetch_array($result))
			if ( strlen(trim($comp["name"])))				
				$lst.='<tr align="left"><td><li type="square"><a href="'.((strlen(trim($comp["href"]))>0) ? $comp["href"] : $_SERVER['PHP_SELF'].'?company='.$comp["id"]).'" class="desktop_list_a">'.$comp["name"].'</a></li></td></tr>';		
		mysql_close($connect);	
...
function assemble_table_person_OtherCompany(&$lst,$company,$findtema='0',$findstr='',$write=false){
	global $config, $lang;
	...
	if(QueryScript("select * from tblOtherCompany where id=$company order by orders,name",$result,$connect)){
		$fetch=mysql_fetch_array($result);
		if(strlen(trim($fetch["comment"]))>0){

			$lst = '<table  bgcolor="'.def_color_desktop_bgcolor.'" width="500" border="1" cellpadding="3" cellspacing="0"  bordercolor="'.def_color_bg.'" align="center">
					<tr><td align="center">'.$fetch["comment"].'</td>
					</tr></table>';
			$lst.='<br>';
	...
	if(QueryScript("select * from tblDepartment order by orders,name",$result_dep,$connect)){
		...
		while ($departament=mysql_fetch_array($result_dep)){
			...
			if(QueryScript("SELECT pe.id, pe.name, pe.email, pe.tel_in,pe.tel_out ,po.name as namepost 
							FROM tblPersonOtherCompany pe, tblPost po 
							WHERE po.id = pe.postid and pe.companyid=".$company." and pe.departid = ".$departament["id"]." and ".$tema_mas[(int)$findtema]." like '".$findstr."%' 
							ORDER BY pe.orders, pe.name	",$result_te,$connect))
			{
				...
				while ($temail=mysql_fetch_array($result_te)){
					...
					if($write && $_SESSION['write']) 
						$lst.='<a href="'.$_SERVER["PHP_SELF"].'?company='.$company.'&edit='.$temail["id"].'" class="desktop_list_a">'.++$number.'</a>';		
					else
						$lst.= ++$number;
					$lst.='</td>
					...
					if($write && $_SESSION['write']) {
						$lst.='<a href="'.$_SERVER["PHP_SELF"].'?company='.$company.'&edit='.$temail["id"].'" class="desktop_list_a">'.$temail["name"].'</a>';		
					}else{
						$lst.=$temail["name"];
					}						
	...
	$lst.='<br><center><a href="'.$_SERVER["PHP_SELF"].'?company='.$company.'" class="desktop_list_a">'.($findstr==''? '' : $lang["tel_mail_alllist"]).'</a></center><br>';		

}

function assemble_cod(&$lst,$company,$write){
	
	if(QueryScript("select * from tblPersonOtherCompany where companyid=".$company." order by orders,name",$result,$connect) &&
	   QueryScript("select * from tblOtherCompany where id=".cod_company." order by orders,name",$result_com,$connect)	){
		
		$lst='';
		$com=mysql_fetch_array($result_com);
		if(strlen(trim($com["comment"]))>0){
			$lst.= '<table  bgcolor="'.def_color_desktop_bgcolor.'" width="500" border="1" cellpadding="3" cellspacing="0"  bordercolor="'.def_color_bg.'" align="center">
					<tr><td align="center">'.$com["comment"].'</td>
					</tr></table>';
		}

		$lst.= '<table  bgcolor="'.def_color_desktop_bgcolor.'" width="500" border="1" cellpadding="3" cellspacing="0"  bordercolor="'.def_color_bg.'" align="center">
				<tr><td align="center" width="70%"><b>'.$lang["tel_mail_object"].'</b></td><td align="center"><b>Код</b></td></tr>';

		while($cod=mysql_fetch_array($result)){ 
			$lst.='<tr><td align="left" width="70%">';
			if($write && $_SESSION['write']) {
				$lst.='<a href="'.$_SERVER["PHP_SELF"].'?company='.$company.'&edit='.$cod["id"].'" class="desktop_list_a">'.$cod["name"].'</a>';		
			}else{
				$lst.=$cod["name"];
			}						
			$lst.='</td><td align="center">'.$cod["tel_in"].'</td></tr>';
		}
		$lst.='</table>';
...
function assemble_temas(&$tems,$company,$findtema='0',$findstr=''){
	global $config, $lang;
...
function assemble_form_search(&$fnd,$company,$findtema='0',$findstr=''){
	global $lang;
...
function assemble_new_tel_email(&$editemail,$company){
	global $config, $lang;

	if(QueryScript("select * from tblDepartment order by orders,name",$result_dep,$connect) &&
		QueryScript("select * from tblPost order by orders,name",$result_post,$connect) &&
		QueryScript("select * from tblOtherCompany order by orders,name",$result_comp,$connect)  ){

		$editemail='<table  bgcolor="'.def_color_desktop_bgcolor.'" width="500" border="0" cellpadding="3" cellspacing="0"  align="center">
					<form action="'.$_SERVER["PHP_SELF"].'?company='.$company.'&action=new" method="post" name="new" id="new">
		...
		while ($postid=mysql_fetch_array($result_post)){
		 ...
		 $editemail.='</select></td></tr>
					<tr><td width="200" align="right">'.$lang["tel_mail_email"].'</td>
						 <td width="300" align="left"><input type="text" size="35" maxlength="255" name="email" value="@so-cdu.ru" style="'.def_style_form_font_size.'"></td></tr>
		...
		$editemail.='</select></td></tr>					
					<tr><td width="200" align="right">'.$lang["other_te_company"].'</td>
						 <td width="300" align="left"><select name="companyid" width="200" size="1" style="'.def_style_form_font_size.'">';
	
		while ($comp=mysql_fetch_array($result_comp)){
			$editemail.='<option value="'.$comp["id"].'"  '.($comp["id"]==$company ? ' selected ' : ' ').' >'.$comp["name"];
		}					 
		$editemail.='</select></td></tr>		 					 
...
function assemble_edit_tel_email(&$editemail,$company){
	global $config, $lang;

	if( QueryScript("select * from tblDepartment order by orders,name",$result_dep,$connect) &&
		QueryScript("select * from tblPost order by orders,name",$result_post,$connect)  &&
		QueryScript("select * from tblOtherCompany order by orders,name",$result_comp,$connect)  &&
		QueryScript("select * from tblPersonOtherCompany where id=".$_GET["edit"]." order by orders,name",$result_te,$connect) )
	{
		...
		if (mysql_num_rows($result_te) != 1){
			...
			$editemail='<table  bgcolor="'.def_color_desktop_bgcolor.'" width="500" border="0" cellpadding="3" cellspacing="0" align="center">
						<form action="'.$_SERVER["PHP_SELF"].'?company='.$company.'&edit='.$_GET["edit"].'&action=edit" method="post" name="edit" id="edit">
...
function is_exist_company(&$company, &$caption){

	if(QueryScript("select * from tblOtherCompany where id=".$company,$result,$connect) ){

		if (mysql_num_rows($result) != 1){
			$company=0;		
			$caption='';
		}else{			
			$oc=mysql_fetch_array($result);
			$caption=$oc["name"];
		}	
...
isset($_GET["company"]) ? $company=$_GET["company"] : $company=0;
...
if($write && isset($_GET["action"])){
	switch($_GET["action"]){
		case 'del':	
			if(isset($_GET["edit"]))
				ExecScript("delete from tblPersonOtherCompany where id=".$_GET["edit"]);	
			break;	
		case 'new':	
			ExecScript('INSERT INTO tblPersonOtherCompany (name, postid, email, tel_in, tel_out, departid, companyid, orders) VALUES ("'.htmlspecialchars($_POST['name']).'",'.$_POST['postid'].',"'.htmlspecialchars($_POST['email']).'","'.htmlspecialchars($_POST["tel_in"]).'","'.htmlspecialchars($_POST["tel_out"]).'",'.$_POST['departid'].','.$_POST["companyid"].','.$_POST['orders'].')');
			break;	
		case 'edit': 
			if(isset($_GET["edit"]))
				ExecScript('UPDATE tblPersonOtherCompany SET name="'.htmlspecialchars($_POST['name']).'", postid='.$_POST['postid'].', email="'.htmlspecialchars($_POST['email']).'", tel_in="'.htmlspecialchars($_POST["tel_in"]).'", tel_out="'.htmlspecialchars($_POST["tel_out"]).'", departid='.$_POST['departid'].', companyid='.$_POST["companyid"].', orders='.$_POST['orders'].'  WHERE id='.$_GET["edit"]);	
			break;	
	}
	header("Location: ".$_SERVER['PHP_SELF'].'?company='.$company);	
}


if($write && $_SESSION["write"]){
	if(!isset($_GET["edit"]) || $_GET["edit"]<1){ 	
		assemble_new_tel_email($editemail,$company);
		$X_desktop[]=array(
		...
		$editemail='edit';
		assemble_edit_tel_email($editemail,$company);
		$X_desktop[]=array(
			"caption"=>$lang["tel_mail_caption_form_edit"],
			"list"=>$editemail,
			"bottom"=>'<a href="'.$_SERVER["PHP_SELF"].'?company='.$company.'" class="desktop_fun_a">'.$lang["tel_mail_form_insert"].'</a>&nbsp;&nbsp;&nbsp;
			<a href="'.$_SERVER["PHP_SELF"].'?company='.$company.'&edit='.$_GET["edit"].'&action=del" class="desktop_fun_a">'.$lang["tel_mail_delete_custom"].'</a>'
		);		
	}
}

if ($company!=cod_company)  
	is_exist_company($company,$caption);

if($company==cod_company){
	assemble_cod($lst,$company,$write);
	$caption = $lang["tel_mail_title_auto_code"];
	$all_lst = '<br>'.$lst.'<br>';	
	$bottom = '<a href="'.$_SERVER["PHP_SELF"].'" class="desktop_fun_a">'.$lang["tel_mail_return_list"].'</a>';		
}elseif($company==0){
	assemble_list_OtherCompany($lst,$write);	
	$caption = $lang["tel_mail_title_emailtel"];
	$all_lst = '<br>'.$lst.'<br>';
	$bottom = "";	
}else{
	assemble_form_search($formfind,$company,$findtema,$findstr);
	assemble_temas($tems,$company,$findtema,$findstr);
	assemble_table_person_OtherCompany($lst,$company,$findtema,$findstr,$write);	
	$all_lst = '<br>'. $formfind.$tems.'<br>'.$lst;
	$bottom = '<a href="'.$_SERVER["PHP_SELF"].'" class="desktop_fun_a">'.$lang["tel_mail_return_list"].'</a>';		
} 
